...
interface SalesHistoryResponse {
  cardResponseList: SalesHistoryItem[];
...
// 판매 내역 API 함수
const getSalesHistory = async (): Promise<SalesHistoryResponse> => {
  const queryParams = new URLSearchParams();
  queryParams.append('cardCategory', 'BUY');

  const response = await api.get<ApiResponse<SalesHistoryResponse>>(
...
export default function SalesHistoryPage() {
  ...
  // 판매 내역 데이터 로드
  const loadSalesHistory = async (status: string) => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  }, [isLoading, error, salesHistory, activeTab]);

  const handleCardClick = (item: SalesHistoryItem) => {
    // SalesHistoryItem을 HistoryItem으로 변환
    ...
    <div className="hidden md:block mb-8">
      ...
      <nav aria-label="페이지 네비게이션">
        <div className="flex items-center gap-2 mb-4">
          <Link
            href="/mypage"
            className="text-gray-500 hover:text-gray-700 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 rounded"
          >
            마이페이지
          </Link>
  ...
  const ErrorState = () => (
    <div className="p-6">
      <div className="text-center py-8">
        ...
        <button
          onClick={() => loadSalesHistory(activeTab)}
                    ...
                    <div className="p-6">
                      {salesHistory?.length > 0 ? (
                        <div
                          className="space-y-4"
                          role="list"
                          aria-label={`${activeTab === 'all' ? '전체' : activeTab === 'selling' ? '판매 중' : '판매 완료'} 판매 내역`}
                        >
                          {salesHistory.map((item) => (
                            <div
                              ...
                              role="listitem"
                              tabIndex={0}
                              className="bg-gray-50 rounded-lg p-4 flex items-start gap-3 cursor-pointer hover:bg-gray-100 focus:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors"
                              onClick={() => handleCardClick(item)}
                              onKeyDown={(e) => handleCardKeyDown(e, item)}
                              aria-label={`${item.carrier} ${item.dataAmount}GB 판매 내역 - ${new Date(item.createdAt).toLocaleDateString('ko-KR')} - ${item.price.toLocaleString()}원 - ${item.sellStatus === 'SELLING' || item.sellStatus === 'PURCHASING' ? '판매중' : '거래완료'}`}
                            >
                              {/* 아이콘 */}
                              <div
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2">
                                  <span
                                    ...
                                    aria-label={`상태: ${item.sellStatus === 'SELLING' || item.sellStatus === 'PURCHASING' ? '판매중' : '거래완료'}`}
                                  >
                                    {item.sellStatus === 'SELLING' ||
                                    item.sellStatus === 'PURCHASING'
                                      ? '판매중'
